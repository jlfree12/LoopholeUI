...
enum ProviderMode: String, Codable, CaseIterable, Identifiable {
    ...
    var subtitle: String {
        ...
        case .anthropic:
            return "Runs the live Loophole workflow against Anthropic so each round is generated fresh."
        }
    }
}
...
enum LoopholeClientError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingAPIKey:
            return "Add an Anthropic API key in Settings before starting a live session."
        case .malformedResponse:
